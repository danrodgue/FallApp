...
- `1:N` con `votos` (un usuario puede votar múltiples ninots con diferentes tipos)
...
Figuras artísticas y construcciones. Los ninots pueden recibir votos individuales
de usuarios en diferentes categorías (favorito, ingenioso, crítico, artístico).
...
Sistema de votación comunitaria de ninots individuales.

```
Columns:
├── id_voto (PK)            | SERIAL (AUTO)
├── id_usuario (FK)         | INT NOT NULL (refs usuarios.id_usuario)
├── id_ninot (FK)           | INT NOT NULL (refs ninots.id_ninot)
├── tipo_voto               | ENUM('favorito', 'ingenioso', 'critico', 'artistico', 'rating')
...
├── id_ninot
├── tipo_voto
└── UNIQUE(id_usuario, id_ninot, tipo_voto) para evitar duplicados
```

**Constraint**: Un usuario puede votar **una sola vez por tipo por ninot**.
...
- `rating`: Valoración general (1-5 estrellas)

**Cambio importante**: Los votos ahora son directamente a **ninots individuales**, 
no a fallas. Cada ninot puede recibir múltiples tipos de votos del mismo usuario.
...
-- Tipos de votos
CREATE TYPE tipo_voto AS ENUM ('favorito', 'ingenioso', 'critico', 'artistico', 'rating');
